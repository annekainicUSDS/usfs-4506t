...
export const address = (() => {
  const countriesArray = countries.map(object => object.value);
  const countriesWithAnyState = Object.keys(states).filter(x => countriesArray.includes(x));
  const countryStateProperties = Object.keys(states).map((key, index) => {
  	return {
	  	properties: {
	      country: {
	        type: 'string',
